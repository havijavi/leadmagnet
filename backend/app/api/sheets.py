...
@router.get("/status", response_model=SheetsStatusOut, dependencies=[Depends(require_any)])
...
@router.get("", response_model=list[SheetsConfigOut], dependencies=[Depends(require_any)])
...
@router.post("", response_model=SheetsConfigOut, status_code=201, dependencies=[Depends(require_admin)])
...
@router.put("/{config_id}", response_model=SheetsConfigOut, dependencies=[Depends(require_admin)])
...
@router.delete("/{config_id}", status_code=204, dependencies=[Depends(require_admin)])
...
@router.post("/{config_id}/sync", dependencies=[Depends(require_member)])
...
@router.post("/sync-all", dependencies=[Depends(require_member)])
